<template>
  <div class="footer">
    <tabbar>
      <tabbar-item @on-item-click="changeState(page.name)" v-for="(page,key) in pages" :key="key" >
        <img slot="icon" :src="page.icon">
        <span slot="label">{{ page.name }}</span>
      </tabbar-item>
    </tabbar>
  </div>
</template>

<script scoped>
  import { Tabbar, TabbarItem } from 'vux';
  export default {
    name: 'common-footer',
    data(){
      return {
        pages: [
          {name: '首页',icon:"data:image/png;base64,"},
          {name: '预约',icon:"data:image/png;base64,"},
          {name: '活动',icon:"data:image/png;base64,"},
          {name: '发现',icon:"data:image/png;base64,"},
          {name: '我的',icon:"data:image/png;base64,"}
        ]
      }
    },
    components: {
      Tabbar,
      TabbarItem
    },
    methods: {
      changeState(name){
        console.log(name);
        let target = '';
        switch (name){
          case '首页':
            target = 'index';
            break;
          case '预约':
            target = 'reserve';
            break;
          case '活动':
            target = 'activity';
            break;
          case '发现':
            target = 'find';
            break;
          case '我的':
            target = 'userCenter';
            break;
        }

        this.$router.push({name: target});
        this.$store.commit('INCREASE_COUNT');
      }
    }
  }
</script>
